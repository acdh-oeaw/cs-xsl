...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:my="myFunctions" version="2.0">
    <!-- 
        ...
        <xsl:if test="@elem or descendant::*[@elem] or $options = 'all'">
            <li>
                <div class="cmds-elem-plus">
                    <span class="detail-caller">
                        <!-- <a href="{concat($detail_model_prefix,@context)}" >  -->
...
sample
 ...
 -->
            ...
            <xsl:when test="$lv = 0">
                <table class="terms-tree show">
                    <thead class="ui-widget-header ui-state-default">
                        <tr>
                            <th class="treecol" rowspan="2">Name <span class="cmd cmd_detail"/>
                            </th>
    ...
    <xsl:template match="Term" mode="terms-tree">
        ...
        <xsl:variable name="is_empty" select="
                if (Term or @count) then
                    ''
                else
                    'empty-term'"/>
        <tr id="{$xid}" class="{$is_empty}">
            <xsl:if
                test="not(parent::Termset or parent::Terms) or parent::Termset[@type = 'dcr' or @type = 'rr']">
            ...
            <td class="treecol">
                <!--<span class="cmd cmd_query" ></span><span class="cmd cmd_columns" ></span>-->
                <span class="column-elem">
                    <xsl:copy-of select="$path_anchored"/>
                </span>
                <!--<span class="cmd cmd_info" ></span>-->
		 ...
		 -->
            <td class="number">
                <!--   <xsl:if test="not(Term)"><xsl:value-of select="@count_text" /></xsl:if>-->
                <xsl:variable name="count_text_sanitized" select="
                        if (not(Term)) then
                            @count_text
                        else
                            sum(descendant-or-self::Term/@count_text)"/>
                <xsl:value-of select="
                        if (number($count_text_sanitized) = number($count_text_sanitized)) then
                            format-number(number($count_text_sanitized), '#.##0', 'european')
                        else
                            ''"/>
            </td>
            <td class="number">
			  ...
			  <xsl:message><xsl:value-of select="concat('formURL: ',my:formURL('values', 'htmllist', concat(@path,'&sort=size')))"></xsl:value-of></xsl:message>
                ...
                <xsl:if test="Term">
                    <xsl:value-of
                        select="format-number(sum(descendant-or-self::Term/@count_distinct_text), '#.##0', 'european')"
                    />
                </xsl:if>
            ...
            <td class="number">
                <xsl:if test="not(Term)">
                    ...
                    <xsl:variable name="inf_content_ratio" select="
                            if (@count_distinct_text != 0) then
                                (@count_text div $count_elems) * (@count_text div @count_distinct_text)
                            else
                                ''"/>
                    <xsl:value-of select="
                            if (number($inf_content_ratio) = number($inf_content_ratio)) then
                                format-number(number($inf_content_ratio), '#.##0,00', 'european')
                            else
                                ''"/>
                </xsl:if>
            </td>
            ...
            <xsl:choose>
                <xsl:when test="$sort = 's'">
                    <xsl:apply-templates select="Term" mode="terms-tree">
                        <xsl:with-param name="parentid" select="$xid"/>
                        <xsl:with-param name="lv" select="$lv + 1"/>
                        <xsl:sort order="descending" select="@count_distinct_text"
                            data-type="number"/>
                    </xsl:apply-templates>
                </xsl:when>
                    ...
                    <xsl:apply-templates select="Term" mode="terms-tree">
                        <xsl:sort select="
                                if (@count_distinct_text != 0) then
                                    ...
                                    0" order="descending" data-type="number"/>
                        <xsl:with-param name="parentid" select="$xid"/>
                        <xsl:with-param name="lv" select="$lv + 1"/>
                    </xsl:apply-templates>
                </xsl:otherwise>
    ...
    <xsl:template match="Termset" mode="select">
        <xsl:variable name="count" select="
                if (Term/@count) then
                    ...
                    ''"/>
        <xsl:variable name="call-value" select="
                if (@type = 'model' or not(@id)) then
                    @name
                else
                    @id"/>
    ...
    <xsl:template name="terms-flat">
        ...
        <xsl:for-each-group
            select=".//Termset[@type = 'model']//Term | .//Termset[@type = 'dcr']/Term | .//Termset[@type = 'rr']//Term[@type = 'rel']"
            ...
            <Term name="{@name}">
                <xsl:for-each select="current-group()[not(@type = 'datcat' or @type = 'rel')]">
                    <context>
                        <!-- <xsl:value-of select="ancestor::Termset[1]/@id" />:  -->
                        <xsl:copy-of select="@*"/>
                        <xsl:value-of select="@path"/>
                    </context>
                </xsl:for-each>
                <xsl:for-each select="current-group()[@type = 'datcat'][parent::Termset]">
                    ...
                    <context>
                        <xsl:copy-of select="@*"/>
                        <xsl:value-of select="ancestor::Termset[1]/@id"/>:<xsl:value-of
                            select="@name"/>
                ...
                <xsl:for-each select="current-group()[@type = 'rel']">
                    <xsl:variable name="datcat" select="
                            if (parent::Term[@type = 'rel']) then
                                parent::Term[@type = 'rel']
                            else
                                ."/>
                    <context>
                        <xsl:copy-of select="@*"/>
                        ...
                        <xsl:for-each select="$datcat//Term[@type = 'datcat']">
                            <!-- [Term/@path] -->
                            <context>
                                <xsl:copy-of select="@*"/>
                                <xsl:value-of select="@path"/>
    ...
    <xsl:template name="list-datcats">
        ...
        <table>
            <caption>DatCats |<xsl:value-of select="count(distinct-values($matrix//Term/@datcat))"
                />| <span class="note">* Click on numbers to see detail </span>
            </caption>
            <thead>
                <tr>
                    <th rowspan="2">id</th>
