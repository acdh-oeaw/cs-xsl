...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    extension-element-prefixes="exsl xs" version="2.0">
    ...
    <xsl:param name="baseurl"
        select="utils:params('baseurl', 'http://193.170.82.207:8985/solr/select?')"/>
    <xd:doc>
        <xd:desc>
            <xd:p>if the base-data with the baseq cannot be retrieved (e.g. network-error) this
                provides a link to the default base-data, which should be a cached version of an
                all-result (<code>*:*</code>) </xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:param name="default-base-data-path"
        select="'http://193.170.82.207:8985/solr/admin/file?file=/data-cache/stats_base.xml'"/>
    <xd:doc>
        <xd:desc>
            <xd:p>prefix for <xd:i>js</xd:i> and <xd:i>css</xd:i> scripts used in html-header</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:param name="scripts-dir"
        select="utils:params('scripts-dir', '../admin/file?file=/scripts/')"/>
    <xd:doc>
        <xd:desc>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>optional string-list to restrict metrics processed from the <xd:a
                    href="http://wiki.apache.org/solr/StatsComponent">stats-component</xd:a>
            </xd:p>
    ...
    <xsl:param name="statsx_metrics" select="utils:params('statsx.metrics', 'sum')"/>
    <xd:doc>
        <xd:desc>
            <xd:p>a multiple of 10 to multiply the relative frequency, i.e. the quotient of absolute
                frequency of the query and the base-query, which is usually quite a small
                number</xd:p>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>a verbose description of the percentile-base, (to be displayed in the output, to
                explain the numbers)</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:param name="percentile-unit" select="'ppm articles'"/>
    <!-- ppm, % -->
    <xsl:param name="all-label" select="'all'"/>
    <!-- ppm, % -->
</xsl:stylesheet>
