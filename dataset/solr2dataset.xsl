...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>sub-stylesheet (of <xd:ref name="amc.xsl"/>) transforming solr facet data to the
                internal representation (&lt;dataset&gt;) </xd:p>
            <xd:p>
    ...
    reldata:<xsl:value-of select="$reldata" />-->
            ...
            <xsl:choose>
                <xsl:when test="//*[contains(@name, 'baseq')] or $reldata = 1">
                    <xsl:call-template name="data2reldata">
                        <xsl:with-param name="query-data" select="$resolved-result//ds:dataset"/>
                    </xsl:call-template>
                </xsl:when>
        ...
        <result dataset_count="{$count_datasets}"
            type="{if ($count_datasets &gt; 1) then 'multiresult' else ''}"
    ...
    <xsl:template name="compute-hits">
        ...
        <xsl:if test="exists($maxScore)">
            ...
            <xsl:variable name="minScore"
                select="$response//result/doc[position() = last()]/*[@name = 'score']"/>
            ...
            <xsl:variable name="estimatedRestScoreSum" select="
                    if ($minScoreRatioed &lt; 1) then
                        ($countRecords - $countRows)
                    else
                        $minScoreRatioed * ($countRecords - $countRows)"/>
            <xsl:message> minScore:<xsl:value-of select="$minScore"/> minScoreRatioed:<xsl:value-of
                    select="$minScoreRatioed"/> estimatedRestScoreSum:<xsl:value-of
                    select="$estimatedRestScoreSum"/>
            </xsl:message>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>resolves multiple queries (based on the <xd:b>qx</xd:b> parameter) with
                subrequests </xd:p>
            <xd:p>it creates a multiresult out of the original result and the results of the
                subrequests.</xd:p>
        </xd:desc>
        <xd:param name="qx">additional queries</xd:param>
        <xd:param name="params">all the parameters of the original result, to be reused in the
            subrequest</xd:param>
    </xd:doc>
            ...
            <result type="multi">
                ...
                <xsl:for-each select="$q-list/*">
                    ...
                    <xsl:variable name="qkey"
                        select="$qkey-list/*[not(. = '')][position() = $curr_pos]"/>
                    ...
                    <xsl:call-template name="subrequest">
                        ...
                        <xsl:with-param name="qkey" select="$qkey"/>
                    </xsl:call-template>
                </xsl:for-each>
            </result>
        </xsl:variable>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>Generates a dataset with multiple queries (based on the <xd:b>qx</xd:b>
                parameter).</xd:p>
            <xd:p>First it creates a multiresult out of the original result and the results of the
                subrequests then lets convert the multiresult into a dataset representation.</xd:p>
        </xd:desc>
        <xd:param name="qx">additional queries</xd:param>
        <xd:param name="params">all the parameters of the original result, to be reused in the
            subrequest</xd:param>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>Although solr's <xd:i>pivot.facet</xd:i> allows pivoting more than two facets,
                this template works only for two facets. However they can be explicitely set
                    (<xd:ref name="facet1" type="parameter"/>, <xd:ref name="facet2"
                    type="parameter"/>).</xd:p>
            <xd:p>all the parameters are optional, by default the template works with the original
                input document (<xd:ref name="source-data" type="parameter"/>) and derives all
                parameters from there.</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template name="pivot2data">
        ...
        <xsl:param name="pivot-fields"
            select="$source-data/response/lst[@name = 'facet_counts']/lst[@name = 'facet_pivot']/arr/@name"/>
        <!-- use qkey as -->
        <xsl:param name="query"
            select="($source-data//lst[@name = 'params']/*[@name = 'qkey'], $source-data//lst[@name = 'params']/*[@name = 'q'])[1]"/>
        ...
        <xsl:param name="facet1-list">
            <xsl:for-each
                select="$source-data/response/lst[@name = 'facet_counts']/lst[@name = 'facet_fields']/lst[@name = $facet1]/int">
        ...
        <xsl:param name="facet2-list">
            <xsl:for-each
                select="$source-data/response/lst[@name = 'facet_counts']/lst[@name = 'facet_fields']/lst[@name = $facet2]/int">
                <xsl:sort select="@name"/>
                <xsl:copy-of select="."/>
            </xsl:for-each>
        </xsl:param>
        <xsl:param name="pivot-data"
            select="$source-data/response/lst[@name = 'facet_counts']/lst[@name = 'facet_pivot']/arr"/>
        <ds:dataset name="{$pivot-fields}-{utils:normalize($query)}"
            label="{$pivot-fields} {$query}">
    ...
    <xsl:template name="facets2labels">
        ...
        <ds:labels>
            ...
            <xsl:for-each select="exsl:node-set($facet-list)/*">
                <ds:label>
                    <xsl:value-of select="
                            if (xs:string(@name) = '') then
                                '_EMPTY_'
                            else
                                translate(xs:string(@name), '~ ', '__')"/>
                </ds:label>
            </xsl:for-each>
        </ds:labels>
    </xsl:template>
    <xd:doc>
        ...
        <xd:param name="facet-list"/>
        <xd:param name="all-value">value for the first column (all), normally
            result/@numFound</xd:param>
    </xd:doc>
    <xsl:template name="facets2dataseries">
        ...
        <ds:dataseries name="{$dataseries-title}">
            <ds:value label="{$all-label}"
                formatted="{utils:format-number($all-value,$number-format-default)}">
                <xsl:value-of select="$all-value"/>
            </ds:value>
            <xsl:for-each select="$facet-list/*">
                <ds:value
                    label="{if(xs:string(@name)='') then '_EMPTY_' else translate(xs:string(@name),'~ ','__')}"
                    formatted="{utils:format-number(.,$number-format-default)}">
                    <xsl:value-of select="."/>
                </ds:value>
            </xsl:for-each>
        </ds:dataseries>
    ...
    <xsl:template match="arr/lst" mode="pivot">
        ...
        <ds:dataseries name="{*[@name = 'value']}">
            <ds:value label="{$all-label}"
                formatted="{utils:format-number(*[@name = 'count'],$number-format-default)}">
            ...
            <xsl:for-each select="exsl:node-set($facet2-list)/*">
                ...
                <xsl:variable name="count"
                    select="$curr_facet/arr[@name = 'pivot']/lst[*[@name = 'value'][. = $curr_label]]/*[@name = 'count']"/>
                <ds:value label="{$curr_label}">
                    <xsl:choose>
                        <xsl:when test="number($count) = number($count)">
                            <!-- test if number -->
                            <xsl:attribute name="formatted">
                                <xsl:value-of
                                    select="utils:format-number($count, $number-format-default)"/>
                            </xsl:attribute>
        ...
        <xsl:for-each select="exsl:node-set($dataseries-list)/*">
            ...
            <ds:dataseries name="{@name}">
                ...
                <xsl:for-each select="exsl:node-set($labels-list)/*">
                    ...
                    <xsl:variable name="count"
                        select="$pivot-data//lst[ancestor-or-self::lst[str[@name = 'field'] = $dataseries and str[@name = 'value'] = $curr_dataseries]][ancestor-or-self::lst[str[@name = 'field'] = $labels and str[@name = 'value'] = $curr_label]]/int[@name = 'count']"/>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>checks if the result isn't already dataset and if not invokes actual
                transformation result2dataset, result-stats2dataset</xd:p>
      ...
      <xsl:when test="exists($result//result/dataset)">
        ...
        -->
            <xsl:when test="exists($result//ds:dataset)">
                <xsl:copy-of select="$result//ds:dataset"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="datasets">
    ...
    <xsl:template name="result2dataset">
        ...
        <xsl:variable name="facet-list">
            <xsl:apply-templates select="exsl:node-set($params)/*[@name = 'facet.field']"
                mode="arrayize"/>
        </xsl:variable>
            ...
            <ds:dataset name="{$curr_facet}">
                <xsl:call-template name="facets2labels">
                    <xsl:with-param name="facet-list">
                        ...
                        <xsl:for-each
                            select="exsl:node-set($result)//response[1]/lst[@name = 'facet_counts']/lst[@name = 'facet_fields']/lst[@name = $curr_facet]/int">
                ...
                <xsl:for-each select="exsl:node-set($result)//response">
                    ...
                    <xsl:variable name="curr_facet_list">
                        <xsl:for-each
                            select="lst[@name = 'facet_counts']/lst[@name = 'facet_fields']/lst[@name = $curr_facet]/int">
                            <xsl:sort select="@name"/>
                            <xsl:copy-of select="."/>
                        </xsl:for-each>
                    </xsl:variable>
                    <xsl:call-template name="facets2dataseries">
                        ...
                        <xsl:with-param name="dataseries-title"
                            select="(.//lst[@name = 'params']/*[@name = 'qkey'], .//lst[@name = 'params']/*[@name = 'q'])[1]"/>
    ...
    <xsl:template name="result-stats2dataset">
        ...
        <xsl:variable name="facet-field-list">
            <xsl:apply-templates select="exsl:node-set($params)/*[@name = 'stats.facet']"
                mode="arrayize"/>
        </xsl:variable>
        ...
        <xsl:for-each select="exsl:node-set($facet-field-list)/*">
            ...
            <xsl:variable name="facet-value-list">
                <xsl:for-each
                    select="exsl:node-set($result)//response[1]/lst[@name = 'stats']/*/lst[1]/lst[@name = 'facets']/lst[@name = $curr_facet]/lst">
                    <xsl:sort select="@name"/>
                    <xsl:copy-of select="."/>
                </xsl:for-each>
            </xsl:variable>
            <ds:dataset name="{$curr_facet}">
                ...
                <xsl:for-each select="exsl:node-set($result)//response">
                    ...
                    <xsl:variable name="metrics"
                        select="distinct-values(lst[@name = 'stats']/*/lst[1]/lst[@name = 'facets']/lst[@name = $curr_facet]/lst/(double | long)/@name)[. = $statsx_metrics or $statsx_metrics = 'all']"/>
                    ...
                    <xsl:for-each select="lst[@name = 'stats']/*/lst">
                        ...
                        <xsl:for-each select="$metrics">
                            ...
                            <xsl:call-template name="facets2dataseries">
                                <xsl:with-param name="dataseries-title"
                                    select="concat($curr_field, '-', $curr_metric)"/>
                                <!-- unifying to the usual structure (element with facet-key in @name-attribute and facet-value as element value -->
                                <xsl:with-param name="facet-list">
                                    <xsl:for-each
                                        select="$stats-data[@name = $curr_field]/lst[@name = 'facets']/lst[@name = $curr_facet]/lst/(double | long)[@name = $curr_metric]">
                                        <ds:value name="{../@name}">
                                            <xsl:value-of select="."/>
                                        </ds:value>
                                    </xsl:for-each>
                                </xsl:with-param>
                                <xsl:with-param name="all-value"
                                    select="$stats-data[@name = $curr_field]/*[@name = $curr_metric]"
                                />
                            </xsl:call-template>
                        </xsl:for-each>
                    </xsl:for-each>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>if a base-query (<xd:ref name="baseq" type="parameter"/>) is provided, enrich the
                query-data with relative frequencies</xd:p>
        </xd:desc>
        <xd:param name="query-data">expects the result already preprocessed in the internal
                <xd:ref>dataset</xd:ref>-representation </xd:param>
        <xd:param name="base-query">the base-query (will be resolved in a separate subrequest
            call)</xd:param>
    </xd:doc>
    ...
    <xsl:template match="ds:dataset" mode="reldata">
        ...
        <ds:dataset name="{$dataset-name}" type="reldata" percentile-unit="{$percentile-unit}">
            ...
            <xsl:variable name="base-dataseries">
                <xsl:variable name="orig-base-dataseries"
                    select="$base-data//ds:dataset[@name = $dataset-name]/ds:dataseries"/>
    ...
    <xsl:template match="ds:value" mode="reldata">
        ...
        <xsl:variable name="base-value"
            select="exsl:node-set($base-data)//ds:value[@label = current()/@label]"/>
        ...
        <ds:value label="{@label}" formatted="{@formatted}" abs="{.}" rel="{$relfreq}"
            ...
            <xsl:value-of
                select="round($relfreq * $percentile-base * $decimal-base) div $decimal-base"/>
        </ds:value>
    </xsl:template>
</xsl:stylesheet>
