...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:utils="http://aac.ac.at/corpus_shell/utils" xmlns:sru="http://www.loc.gov/zing/srw/"
    xmlns:saxon="http://saxon.sf.net/" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:fcs="http://clarin.eu/fcs/1.0" xmlns:ds="http://aac.ac.at/corpus_shell/dataset"
    xmlns:exsl="http://exslt.org/common" version="2.0"
    exclude-result-prefixes="saxon xs exsl diag sru fcs utils ds">
...
<change on="2013-01-24" type="created" by="vr"></change>	
 ...
 -->
    ...
    <xsl:template name="top-menu">
        <xsl:for-each select="//ds:dataset">
            ...
            <xsl:variable name="href">
                <xsl:choose>
                    <xsl:when test="contains($mode, 'multiple-files')">
                        <xsl:value-of select="concat($output_file_prefix, $dataset-key, '.html')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="concat('#table-', $dataset-key)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
    ...
    <xsl:template match="ds:dataset" mode="multi">
        <xsl:variable name="dataset-key" select="utils:dataset-key(.)"/>
        <xsl:variable name="output_href" select="concat($output_file_prefix, $dataset-key, '.html')"/>
