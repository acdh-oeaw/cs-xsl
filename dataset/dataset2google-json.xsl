...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    exclude-result-prefixes="exsl utils xs xd ds">
  ...
  <xsl:import href="amc-helpers.xsl"  />-->
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>sub-stylesheet to produce google-chart out of the internal
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>generate a json-object out of the facet-list provided as parameter and add all
                scripts (js, css) necessary for the visualization</xd:p>
            <xd:p>the chart will be generated inside a div#infovis </xd:p>
            <xd:p>expects div#infovis and still need to invoke init()-function (onload or onclick
                somewhere) </xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template name="chart-google">
        ...
        <xsl:variable name="layout" select="
                if (count($data//ds:dataseries[not(@type = 'base')]) &gt; 1) then
                    'line'
                else
                    'pie'"/>
        <xsl:variable name="legend-position" select="
                if (count($data//ds:dataseries[not(@type = 'base')]) = (2, 3)) then
                    'top'
                else
                    'right'"/>
        ...
        <script type="text/javascript">
            ...
            data[ "<xsl:value-of select="$dataset-name"/>
            "] = google.visualization.arrayToDataTable(<xsl:copy-of select="$json-data"/>
)
            ...
            ',
            legend: {
                position: '<xsl:value-of select="$legend-position"/>
            '}
        }
    ...
    <xsl:template match="ds:dataseries[ds:value]" mode="chart-google">
        <xsl:if test="not(xs:string(@name) = $all-label or @key = $all-label)">
            ...
            <xsl:text>', </xsl:text>
            <xsl:apply-templates mode="chart-google"
                select="ds:value[not(@key = current()/../ds:labels/ds:label[@type = 'base'])]"/>
            <xsl:text>]</xsl:text>
            <xsl:if test="not(position() = last())">, </xsl:if>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="ds:value[not(xs:string(@key) = $all-label)][not(@type = 'base')]"
        mode="chart-google">
        <xsl:value-of select="
                (if (exists(@rel)) then
                    round(number(@rel) * 1.0E10) * 1.0E-4
                else
                    (), @abs, .)[1]"/>
