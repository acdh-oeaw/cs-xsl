...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
                    ...
                    *******************
                    ...
                    <!-\-<xsl:call-template name="formURL">
                        ...
                        <xsl:with-param name="q">
                            <xsl:value-of select="encode-for-uri(replace(replace(ResourceRef,'/','%2F'),'\.','%2E'))"/>
                        </xsl:with-param>
                    </xsl:call-template>-\->
                </xsl:otherwise>
            </xsl:choose>-->
    ...
    <xsl:template match="cmd:IsPartOfList" mode="record-data">
        <div>
            <span class="label">Resource is part of: </span>
            <xsl:apply-templates mode="record-data"/>
        </div>
    </xsl:template>
    <xsl:template match="cmd:IsPartOf" mode="record-data">
        <a class="Metadata" href="{.}" target="_blank">
            <xsl:value-of select="."/>
        </a>
    </xsl:template>
