...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xpath-default-namespace="http://explain.z3950.org/dtd/2.0/">
    <!-- 
    ...
    <xsl:variable name="title"
        select="concat('explain: ', (//databaseInfo/title[@lang = $lang]/text(), //databaseInfo/title/text(), $site_name)[1])"/>
    ...
    <xsl:template match="databaseInfo"> databaseInfo <h2>
    ...
    <xsl:template match="index">
        ...
        <xsl:param name="format" select="
                if ($format = 'htmllist') then
                    'html'
                else
                    $format"/>
        <xsl:variable name="scan-index"
            select="concat('fcs?operation=scan&amp;scanClause=', map/name , '&amp;x-context=', $x-context, '&amp;x-format=', $format )"/>
        <xsl:choose>
