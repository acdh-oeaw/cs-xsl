...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:cr="http://aac.ac.at/content_repository"
    ...
    extension-element-prefixes="sru fcs utils xs xd">
    <xsl:import href="scan2view_v1.xsl"/>
    <xsl:import href="../commons_v2.xsl"/>
    <xd:doc>
        <xd:desc>generate a view for a values-list (index scan) <xd:p>Note: This is called eg. from
                fsc:scan with mode "subsequence".</xd:p>
            <xd:p>
                <xd:pre>
...
-->
    ...
    <xsl:template name="continue-root">
        <div class="scan-index-{translate($index,'.','-')}">
                    ...
                    <xsl:variable name="q">
                        <xsl:choose>
                            <xsl:when test="sru:extraTermData/cr:type">
                                <xsl:value-of
                                    select="concat(sru:extraTermData/cr:type, '%3D%22', sru:value, '%22')"
                                />
                            </xsl:when>
                            <xsl:when test="ancestor::sru:term">
                                <xsl:variable name="group-term" select="ancestor::sru:term[1]"/>
                                <xsl:value-of
                                    select="concat($group-term/sru:extraTermData/cr:type, '%3D%22', $group-term/sru:value, '%22', ' and ', $index, '%3D%22', sru:value, '%22')"
                                />
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="concat($index, '%3D%22', sru:value, '%22')"/>
        ...
        <xsl:variable name="link">
            <span>
                ...
                <a class="value-caller" href="{$href}">
                    <!--target="_blank"-->
                    <xsl:value-of select="
                            if (normalize-space((sru:displayTerm, sru:value)[1]) eq '') then
                                '----------'
                            else
                                (sru:displayTerm, sru:value)[1]"/>
                </a>
            </span>
