...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:fcs="http://clarin.eu/fcs/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" version="1.0">
    <xsl:import href="../commons_v1.xsl"/>
    <xd:doc scope="stylesheet">
        <xd:desc> generate a view for a values-list (index scan) <xd:p>History:
                        <xd:ul><xd:li>2012-02-06: created by:"vr": from values2view.xsl, from
                        model2view.xsl</xd:li></xd:ul></xd:p><xd:p><xd:pre>
...
-->
    ...
    <xd:doc>
        <xd:desc>The index is defined as the part of the scanClause before the '=' <xd:p> This is
                ...
                a complete index, relation, term clause in CQL". </xd:p><xd:p> Note: for the special
                scan clause fcs.resource this is an empty string. See <xd:a
                    href="http://www.w3.org/TR/xpath/#function-substring-before">.XPath language
                    definition</xd:a></xd:p></xd:desc>
    </xd:doc>
    ...
    <xsl:variable name="responsePosition"
        select="/sru:scanResponse/sru:echoedScanRequest/sru:responsePosition"/>
    <xsl:variable name="maximumTerms"
        select="/sru:scanResponse/sru:echoedScanRequest/sru:maximumTerms"/>
    <xsl:variable name="scanClause" select="/sru:scanResponse/sru:echoedScanRequest/sru:scanClause"/>
    <xd:doc>
        <xd:desc>Standard callback from / template <xd:p><xd:ul><xd:li>If a htmlpage is requested
                        generates input elements for the user to do another
                        scan.</xd:li><xd:li>Wraps the HTML representation of the result terms in an
                        HTML div element.</xd:li></xd:ul></xd:p></xd:desc>
    </xd:doc>
    ...
    <xsl:template name="header">
        ...
        <div class="header"><xsl:attribute name="data-countTerms"><xsl:value-of select="$countTerms"
                /></xsl:attribute><xsl:attribute name="startItem"><xsl:value-of
                    select="$responsePosition"/></xsl:attribute><xsl:attribute name="maximumTerms"
                    ><xsl:value-of select="$maximumTerms"/></xsl:attribute>
            <!--<xsl:value-of select="$title"/>--><form><input type="text" name="index"
                    value="{$index}"/><input type="text" name="scanClause" value="{$filter}"/><input
                    type="hidden" name="operation" value="scan"/><input type="hidden"
                    name="x-format" value="{$format}"/><input type="hidden" name="x-context"
                    value="{$x-context}"/><input type="submit" value="suchen"/><xsl:call-template
                    name="prev-next-terms"/></form><xsl:value-of
                select="count(//sru:terms/sru:term)"/> out of <xsl:value-of select="$countTerms"/>
            Terms </div>
    </xsl:template>
    <xsl:template name="prev-next-terms">
        <xsl:variable name="prev_responsePosition">
            <xsl:choose>
                <xsl:when test="number($responsePosition) - number($maximumRecords) &gt; 0">
                    <xsl:value-of
                        select="format-number(number($responsePosition) - number($maximumRecords), '#')"
                    />
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="next_responsePosition">
            <xsl:choose>
                <xsl:when
                    test="number($responsePosition) + number($maximumRecords) &gt; number(numberOfRecords)">
    ...
    <xd:doc>
        <xd:desc>A term consits of a number for this term and the term itself <xd:p>The term is
                presented as a link that can be used to scan for that term.</xd:p><xd:p> Sample
                data: <xd:pre>
            &lt;sru:term&gt;
    ...
    <xsl:template match="sru:term">
        ...
        <xsl:variable name="link">
            <span>
                ...
                <a class="value-caller" href="{$href}">
                    <!--target="_blank"-->
