...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:zr="http://explain.z3950.org/dtd/2.0/"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>generate a view for the explain-record
            (http://www.loc.gov/standards/sru/specs/explain.html) </xd:desc>
    </xd:doc>
    <xd:doc>
        <xd:desc/>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Called from common_v1.xsl to present a title string <xd:p> Note: only called if a
                complete page is requested. </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Entry point called from commons_v1.xsl <xd:p> If the requested format contains
                table try to display as much information as we can else this is most likely an ajax
                request so repsond just with the heading and the list of possible indexes. </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xsl:template match="sru:version" mode="verbose">
        <div class="sru-version"> SRU version: <xsl:value-of select="."/>
        </div>
    </xsl:template>
    <xsl:template match="sru:recordPacking" mode="verbose">
        <div class="sru-recordPacking"> Default record packing: <xsl:value-of select="."/>
        </div>
    </xsl:template>
    <xsl:template match="sru:recordSchema" mode="verbose">
        <div class="sru-recordSchema"> Record schema: <xsl:value-of select="."/>
        </div>
    </xsl:template>
    ...
    <xd:doc>
        <xd:desc>In verbose mode first display the list and then the items only generated in this
            mode</xd:desc>
    </xd:doc>
    ...
    <xsl:template match="zr:configInfo" mode="verbose">
        <div class="zr-serverInfo">
            <p>Default number of records: <xsl:value-of
                    select="zr:default[@type = 'numberOfRecords']"/>
            </p>
            <p>Settings: maximum records returned: <xsl:value-of
                    select="zr:setting[@type = 'maximumRecords']"/>, result TTL: <xsl:value-of
                    select="zr:setting[@type = 'resultSetTTL']"/>
            </p>
        </div>
    </xsl:template>
    <xd:doc>
        <xd:desc>In verbose mode even give hints to unprocessed nodes by including their text as
            comment</xd:desc>
    </xd:doc>
