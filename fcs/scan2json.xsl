...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc> generate a json object of the scanResponse <xd:p>Output:</xd:p>
            <xd:pre>
...
&lt;sru:version&gt;1.2&lt;/sru:version&gt;
 ...
 &lt;/sru:scanResponse&gt;
    ...
    <xd:doc>
        <xd:desc>Converts a single term to json format <xd:p> sample data: </xd:p>
            <xd:pre>
        &lt;sru:term&gt;
...
"terms": [
    ...
    <xsl:template match="sru:term">
        ...
        <xsl:value-of select="translate(normalize-space($display), '&#34;', '')"/> | <xsl:value-of
            select="sru:numberOfRecords"/>
        <xsl:text>|", </xsl:text>
