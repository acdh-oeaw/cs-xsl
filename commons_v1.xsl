...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:cr="http://aac.ac.at/content_repository"
    ...
    xmlns:diag="http://www.loc.gov/zing/srw/diagnostic/" xmlns:sru="http://www.loc.gov/zing/srw/"
    xmlns:fcs="http://clarin.eu/fcs/1.0" xmlns:exsl="http://exslt.org/common"
    xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" version="1.0"
    extension-element-prefixes="diag sru fcs exsl">
    ...
    <xd:doc>
        <xd:desc>Common starting point for all stylesheet <xd:p>Cares for unified html-envelope and
                passes back to the individual stylesheets for the content (via template: <xd:ref
                    name="continue-root" type="template">continue-root</xd:ref>) </xd:p>
            <xd:p> Depending on <xd:ref name="format" type="parameter">$format</xd:ref> containing
                that name generates the html framework using one of the following named templates <xd:ul>
                    <xd:li>
                        <xd:ref name="html" type="template">html</xd:ref> for ??? </xd:li>
                    <xd:li>
                        <xd:ref name="htmljs" type="template">htmljs</xd:ref> for ??? </xd:li>
                    <xd:li>
                        <xd:ref name="htmlsimple" type="template">htmlsimple</xd:ref> for ???
                    </xd:li>
                </xd:ul> If none of these are found only the contents generated by <xd:ref
                    name="continue-root" type="template">continue-root</xd:ref> are returned.
            </xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="/">
    ...
    <xd:doc>
        <xd:desc>Generates the typical html framework and integrates the parts created by the
            specialised named templates</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Generates the typical html framework and integrates the more parts created by the
            specialised named templates</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>An html-envelope for a simple (noscript) view <xd:p> Uses
                $script_url/style/cmd-ui.css </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Do-nothing definition of callback-header. Overwrite this in specialised
            templates</xd:desc>
    </xd:doc>
    <xsl:template name="callback-header"/>
    <xd:doc>
        <xd:desc>Template used to display an error/diagnostic message returned by the upstream
            endpoint <xd:p> The error message can be found in diag:diagnostic so the template for
                this is directly related. </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Fetches the context from the URL provided in $context_url <xd:p> $context_url is
                influenced by <xd:ref name="base_url" type="variable">$base_url</xd:ref>. </xd:p>
            <xd:p> Note: by default $base_url is set to an empty string so the URL is assumed to be
                wherever these style sheet is executed. This may lead to warnings by the XSLT
                processor. </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        ...
        <xd:param name="action">Same meaning as <xd:ref name="operation" type="parameter"
                >$operation</xd:ref>. Defaults to <xd:ref name="operation" type="parameter"
                >$operation</xd:ref>.</xd:param>
        <xd:param name="format">Same meaning as <xd:ref name="format" type="parameter"
                >$format</xd:ref>. Defaults to <xd:ref name="format" type="parameter"
                >$format</xd:ref>.</xd:param>
        <xd:param name="q">Same meaning as <xd:ref name="q" type="parameter">$q</xd:ref>. Defaults
            to <xd:ref name="q" type="parameter">$q</xd:ref>.</xd:param>
        <xd:param name="startRecord">Same meaning as <xd:ref name="startRecord" type="parameter"
                >$startRecord</xd:ref>. Defaults to <xd:ref name="startRecord" type="parameter"
                >$startRecord</xd:ref>.</xd:param>
        <xd:param name="maximumRecords">Same meaning as <xd:ref name="maximumRecords"
                type="parameter">$maximumRecords</xd:ref>. Defaults to <xd:ref name="maximumRecords"
                type="parameter">$maximumRecords</xd:ref>.</xd:param>
    </xd:doc>
    <xsl:template name="formURL">
        ...
        <xsl:choose>
            <xsl:when test="$action = 'get-data'">
                <xsl:value-of
                    select="concat($base_url, 'get/', $q, '/data', translate($param_format,'&amp;','?'))"
                />
            </xsl:when>
            <xsl:when test="$action = 'get-metadata'">
                <xsl:value-of
                    select="concat($base_url, 'get/', $q, '/metadata/', $md-format, translate($param_format,'&amp;','?'))"
                />
            </xsl:when>
            <xsl:when test="$action = 'explain'">
                <xsl:value-of
                    select="concat($base_url, 'fcs?version=1.2&amp;operation=',$action, $param_x-context, $param_format)"
                />
            </xsl:when>
            <xsl:when test="$action = 'scan'">
                <xsl:value-of
                    select="concat($base_url, 'fcs?version=1.2&amp;operation=',$action, $param_scanClause, $param_x-context, $param_format, $param_maximumTerms)"
                />
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('?version=1.2&amp;operation=',$action, $param_q, $param_x-context, $param_startRecord, $param_maximumRecords, $param_format)"
                />
            </xsl:otherwise>
        </xsl:choose>
...
-->
    ...
    <xd:doc>
        <xd:desc>Add link to more information to a link using information from mappings and some XML
            attribute key</xd:desc>
        <xd:p> Note: The context element where on which this template is applied is assumed to have
            that key XML attribute. </xd:p>
    ...
    <xd:doc>
        <xd:desc>Formats an XML fragment so it's readable as such (using mode format-xmlelem), some
            elements as list or a single element using <xd:ref name="format-value" type="template"
                >format-value</xd:ref>
        </xd:desc>
    ...
    <xd:doc>
        <xd:desc>Format values eg. when formatting attributes like below <xd:p> This basic
                implementation just replaces http: containing URLs with a matching HTML a tag that
                opens the link in another window. </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Provides a generic html-view for xml-elements </xd:desc>
        <xd:param name="strict">xs:boolean: Stay in mode format-xmlelem mode (or try to go back to
            mode=record-data)</xd:param>
    </xd:doc>
    <xsl:template match="*" mode="format-xmlelem">
        ...
        <xsl:if test=".//text() or @*">
            <xsl:variable name="has_text">
                <xsl:choose>
                    <xsl:when test="normalize-space(text()[1]) = 'Unspecified'"
                        >unspecified</xsl:when>
    ...
    <xsl:template match="@*" mode="format-attr">
        <span class="test inline label">@<xsl:value-of select="name()"/>: </span>
        <span class="value">
            <xsl:call-template name="format-value"/>
            <!--<xsl:value-of select="." /> -->
        </span>
